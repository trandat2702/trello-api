...
import { EMAIL_RULE, OBJECT_ID_RULE, OBJECT_ID_RULE_MESSAGE } from '~/utils/validators'
...
const CARD_COLLECTION_SCHEMA = Joi.object({
  ...
  comments: Joi.array().items({
    ...
    content: Joi.string(),
    //chỗ này cần lưu ý vì dùng hàm $push để them comment nên không set default Date.now() được, chúng ta sẽ set default Date.now() luôn giống hàm insertOne khi create được
...
const update = async (cardId, updateData) => {
  try {
    ...
    //Đối với những dữ liệu liên quan đến ObjectId thì chúng ta cần convert nó về dạng ObjectId trước khi lưu
